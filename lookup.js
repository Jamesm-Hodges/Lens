...
// ── Live fee-claim detection (precise, on-demand) ──
// Cached bankr_claim_history is only as fresh as the cron. To make a scan reflect
// reality immediately, detect claims on-chain here too. A Bankr/Doppler fee claim
// pays the recipient BOTH WETH and the project token in the SAME tx — we match on
// that. Returns { count, weth, lastTs }. Falls back silently on RPC errors.
async function computeClaimsLive(tokens) {
  const WETH_BASE = '0x4200000000000000000000000000000000000006';
  let count = 0, weth = 0, lastTs = 0;
  ...
  for (const token of (tokens || []).slice(0, 4)) {
    ...
    try {
      ...
      for (const t of (wethRes?.transfers || [])) {
        ...
        if (!h) continue;
        wethByTx.set(h, (wethByTx.get(h) || 0) + (t.value || 0));
      }
      const seenTx = new Set();
      for (const t of (tokenRes?.transfers || [])) {
        const h = String(t.hash || '').toLowerCase();
        if (!h || seenTx.has(h) || !wethByTx.has(h)) continue;
        seenTx.add(h);
        count++;
        weth += wethByTx.get(h);
...
async function lookupProfile({ username, wallet }) {
  ...
  // Live on-chain claim detection takes precedence over the (cron-cached) table so a
  // fresh claim shows up immediately and precisely. Fall back to cache if live finds none.
  ...
  return {
    ...
    claims: { has_claimed: hasClaimed, total_eth_claimed: totalClaimedEth.toFixed(4), claim_count: claimCount, last_claimed_at: lastClaimedAt, claim_history: claimData.slice(0, 5) },

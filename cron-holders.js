...
async function checkAndSaveClaims(wallet, tokenAddress, tokenSymbol) {
  try {
    ...
    //
    // Precise signature of a fee claim: in the SAME transaction the recipient wallet
    // receives BOTH WETH and the project token from the fee contract. We pull WETH-in
    // and token-in, group by tx hash, and count a claim only when both are present in
    // the same tx. This isolates claims from plain WETH receipts (swaps) and from buys
    // (token in, but WETH out — so no WETH-in row for that tx).
    ...
    // claim = tx where the project token AND WETH both arrived
